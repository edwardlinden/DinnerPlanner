...
class SidebarView {
    constructor (container, model) {
		...
		this.model=model;
		model.notifyObserver(this.update);
		...
		this.plusButton.addEventListener("click", numGuestController);
		this.minusButton = container.querySelector("#minusGuest");
		
		function numGuestController(){
			let numGuest = model.setNumberOfGuests(model.getNumberOfGuests() + 1);
	...
	update() {
		this.numberOfGuests.innerHTML = numGuestController();
	}
